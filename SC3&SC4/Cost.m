...
valid_ = max(secondly_matrix_repeated(21600:53053+21600-1))-secondly_matrix_repeated(21600:53053+21600-1);
valid_1 = valid_; valid_2 = valid_;
valid_1(valid_1<Charg_rates(x_(4))) = 0;
valid_2(valid_1<Charg_rates(x_(5))) = 0;
%%
deposit_energy = 78; deposit_SOC = 100*deposit_energy/x_(1);
% x = [534.277535446490	95	80	2	2	88	36	74	21	6665	44033];
X = x_(6:end);
counter = 0;
for i=1:size(StNumMat,2)
    for j=1:size(StNumMat{i},2)
        ...
        % if (2*deposit_energy + 750 - charge_cumsum)/7 > DepotCharger_rate(x_(192))
        %     J=inf;
        %     return;
        % end
...
while i<L-1
    ...
    for j = 1:numel(Bus_Num_Mat)
        for k = 1:Bus_Num_Mat(j)
            ...
            if (((StNumMat{j}{k}(i)==Laststations(1)) && valid_1(i)) || ((StNumMat{j}{k}(i)==Laststations(2)) && valid_2(i))) && ...
                ...
                if (StNumMat{j}{k}(i)==Laststations(1) && SumChrg23<ChrgLim23) || (StNumMat{j}{k}(i)==Laststations(2) && SumChrg46<ChrgLim46)                     
                    ...
                    elseif  FlagMinBus && (allowChtime>0)
                        ...
                        % SOC_final = SOC{j}{k}(i+PantoChTime);
                        % Cost_deg = deltaE(SOC_ini,SOC_final,E,discount,CC);
                        % Cost_deg_matrix{j}{k}(i+1:i+PantoChTime) = (Cost_deg./(PantoChTime));
                        % SpeedMat{j}{k}(i+1:i+PantoChTime) = 0;%[zeros(PantoChTime,1); SpeedMat{j}{k}(i+1:end)];
                        % SlopeMat{j}{k}(i+1:i+PantoChTime) = 0;%[repmat(SlopeMat{j}{k}(i+1),PantoChTime,1); SlopeMat{j}{k}(i+1:end)];
                        % PassMat{j}{k}(i+1:i+PantoChTime) = 0;%[zeros(PantoChTime,1); PassMat{j}{k}(i+1:end)];
                        % StNumMat{j}{k}(i+1:i+PantoChTime) = 0;%[repmat(StNumMat{j}{k}(i+1),PantoChTime,1); StNumMat{j}{k}(i+1:end)];
                        ChrgFlag{j}{k}(i+1:i+PantoChTime) = StNumMat{j}{k}(i)*ones(PantoChTime,1);
                        FlagMat{j}{k}(i+1:i+PantoChTime) = 0;%[FlagMat{j}{k}(i)*ones(PantoChTime,1); FlagMat{j}{k}(i+1:end)];
                        % QpartMat{j}{k}(i+1:i+PantoChTime) = 0;%[QpartMat{j}{k}(i)*ones(PantoChTime,1); QpartMat{j}{k}(i+1:end)];
                        % minBusN(end+1,:) = minBusN(end,:) - 1;
                        % Lnew(j,k) = length(ChrgFlag{j}{k});
                    end
                end
            end
        end
...
depotCharger = DepotCharger_rate(DepotCharger_rate>max(depotcharge_)/7.5);
for i=1:13
    if depotcharge_(i)/7.5>depotCharger
        J=inf;
        return
    end
end
...
ChargerCostRF_depot = chargerRF(discount2,DepotCharger_cost(DepotCharger_rate>max(depotcharge_)/7.5));
...
secondly_matrix_repeated = repelem(curi_demand_line1, 1, 3600);
a = secondly_matrix_repeated(21600:53053+21600-1)'+Charger_rate(1)*SumChrg23_+Charger_rate(2)*SumChrg46_;
penalty = 0;
if max(a)>max(curi_demand_line1)
    penalty = (1e8)*(max(a)-max(curi_demand_line1));
end
% J = sum(ChargerCostRF.*(x_(2:3))) + Energy_cost + BatCost + depotcharge*min(tariff) + penalty;

J = sum(ChargerCostRF.*(x_(2:3))) + Energy_cost + BatCost + depotcharge*min(tariff) + demandcost + 13*ChargerCostRF_depot + penalty;
...
totalDemand = secondly_matrix_repeated + [repelem(sum(depotcharge_ / 7), 1, 5*3600),zeros(1,3600),(Charger_rate(1)*SumChrg23_+Charger_rate(2)*SumChrg46_)',zeros(1,16*3600+1-53054),repelem(sum(depotcharge_ / 7), 1, 2*3600)];
X_axis = (1:86400)/3600;
plot(X_axis,secondly_matrix_repeated/1000,'LineWidth',3)
% bar(secondly_matrix_repeated/1000,)
hold on
plot(X_axis,totalDemand/1000,'LineWidth',1)
ylabel('Demand (MW)')
set(gca,'fontsize',12,'fontweight','b')
xlabel('Time (h)');
legend('City Demand','Total Demand')
% 
% offset = 21600;
% fontsize = 12;
% fig = figure;
