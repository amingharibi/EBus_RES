...
constraint2 = x(1:24) <= 0.75*x(25);
constraint3 = x(1:24) >= -0.75*x(25);

for i=1:23
    const1(i) = x(28) + sum(x(1:i));
end
const1(24) = sum(x(1:24));
...
prob.Constraints.cons4 = const1>=0.05*x(25);  % Add Constraint2 to Problem
prob.Constraints.cons5 = const1<=0.95*x(25);  % Add Constraint2 to Problem
prob.Constraints.cons6 = PotherGen <= max(curi_demand_line1);
prob.Constraints.cons8 = p_pv - x(1:24) + p_wt <= 1.1*max(Pl);
prob.Constraints.cons9 = sum(x(1:24)) == x(28);

x0.x = UB;
options = optimoptions(@fmincon,'MaxIterations',15000,'display','none');
[sol,fval] = solve(prob,x0,'Options',options);
Best_pos = sol.x;
% fval_ = [fval_;fval];
% Best_pos_ = [Best_pos_;Best_pos];
% end
%% Overall Cost
% J = sum(ChargerCostRF.*(x(4:5))) + BatCost + Best_score;
%% Demands
...
J = sum(ChargerCostRF.*(x_(2:3))) + Energy_cost + BatCost + depotcharge*min(tariff) + fval + demandcost + 13*ChargerCostRF_depot;
J
minBusN
% if J<-39900
% display(['J: ',num2str(J),'   fval: ',num2str(J)])
% end
% if J<-39920
%     ss=2;
% end
...
% [J Best_score J+Best_score]
% J2 = sum(ChargerCostRF.*(x(4:5))) + BatCost + Energy_cost + depotcharge*min(tariff);
% J
%%
offset = 21600;
fontsize = 12;
