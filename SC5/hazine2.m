function [z, pgrid, p_pv, p_wt,dccc_bat NECL TMP]=hazine2(x,Pl1,Pl2,Pl3,curi_demand_line1)
...
pbat = 0.927*x(1:24);
pgrid = pbat - p_wt - p_pv + Pl;
z = sum(pgrid .* Mu);
% PotherGen = Pl + pgrid;
% avarage_cycle = sum(abs(pbat))/x(25);
% Energy_exchanged = sum(abs(pgrid));
